...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    override fun onSensorChanged(event: SensorEvent?) {
        // copy values to class properties
        if (event?.sensor?.type == Sensor.TYPE_ACCELEROMETER) {
            var pitchEstimate = asin(event.values[0]/9.81f)
            var rollEstimate = atan(event.values[1]/event.values[2])

            binding.pitch.text = pitchEstimate.toString()
            binding.roll.text = rollEstimate.toString()
